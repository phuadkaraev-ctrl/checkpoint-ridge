...
| --- | --- | --- | --- |
| System map | Orbit in 3D or use the interactive 2D compatibility map; select seven districts in either view | Checkpoint behavior connects memory, WAL, storage, recovery metadata, and replicas | Concept |
...
| Recovery ground | Play or scrub local WAL replay and inspect the separate HA branch | Checkpoint gap is not recovery duration; primary crash recovery and standby promotion are distinct paths | Observed + concept |
